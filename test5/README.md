...
| 字段 | 类型 | 主键，外键 | 可以为空 | 默认值 | 约束 | 说明 |
| ------ | ------| ------ | ------| ------ | ------ | ----- |
| bro_id | Int |  主键| 否|  |  | 记录存储号 |
| book_id | Int |  外键| 否|  |  | 书籍编号（图书表主键） |
| user_id | Int | 外键 | 否 |  |  | 读者编号（用户表主键） |
...
|参数名称|必填|说明|
| ----- | ----- | ------|
|user_name|是|获取用户的用户名|
|password|是|获取用户的登录密码|
|class|是|获取用户的登录角色|
- 返回实例：
<pre>
<code>
...
|参数名称|说明|
| ----- | ----- |
|info|返回给用户的信息，显示是否登录成功|
|data|根据用户输入，返回相应的首页页面，同时在页面中返回用户的部分信息|
|code|返回码|
#### 2.2 图书管理系统首页设计
![系统首页](./img/sys.jpg)
- 类图参见：读者类，图书类
...
|参数名称|必填|说明|
| ----- | ----- | ------|
|class_id|是|获取用户的登录角色，返回相应的页面,1为学生用户|
